...
replication_started(#rep{id = {BaseId, _} = RepId}) ->
    ...
    #rep_state{rep = #rep{doc_id = DocId}} ->
        ...
        ?LOG_REP("Document `~s` triggered replication `~s`",
...
replication_completed(#rep{id = RepId}, Stats) ->
    ...
    #rep_state{rep = #rep{doc_id = DocId}} ->
        ...
        ?LOG_REP("Replication `~s` finished (triggered by document `~s`)",
...
handle_call(Msg, From, State) ->
    ?LOG_REP_ERROR("Replication manager received unexpected call ~p from ~p",
        [Msg, From]),
...
handle_cast(Msg, State) ->
    ?LOG_REP_ERROR("Replication manager received unexpected cast ~p", [Msg]),
...
handle_info({'EXIT', From, Reason}, #state{db_notifier = From} = State) ->
    ?LOG_REP_ERROR("Database update notifier died. Reason: ~p", [Reason]),
...
handle_info(Msg, State) ->
    ?LOG_REP_ERROR("Replication manager received unexpected message ~p", [Msg]),
...
rep_db_update_error(Error, DocId) ->
    ...
    ?LOG_REP_ERROR("Replication manager, error processing document `~s`: ~s",
        [DocId, Reason]),
...
maybe_start_replication(State, DocId, RepDoc) ->
    ...
    nil ->
        ...
        ?LOG_REP("Attempting to start replication `~s` (document `~s`).",
            [pp_rep_id(RepId), DocId]),
    ...
    #rep_state{starting = false, rep = #rep{doc_id = OtherDocId}} ->
        ?LOG_REP("The replication specified by the document `~s` was already"
            " triggered by the document `~s`", [DocId, OtherDocId]),
        maybe_tag_rep_doc(DocId, RepDoc, ?l2b(BaseId)),
        State;
    #rep_state{starting = true, rep = #rep{doc_id = OtherDocId}} ->
        ?LOG_REP("The replication specified by the document `~s` is already"
...
rep_doc_deleted(DocId) ->
    ...
    [{DocId, RepId}] ->
        ...
        true = ets:delete(?REP_TO_STATE, RepId),
        true = ets:delete(?DOC_TO_REP, DocId),
        ?LOG_REP("Stopped replication `~s` because replication document `~s`"
...
maybe_retry_replication(#rep_state{retries_left = 0} = RepState, Error, State) ->
    ...
    true = ets:delete(?REP_TO_STATE, RepId),
    true = ets:delete(?DOC_TO_REP, DocId),
    ?LOG_REP_ERROR("Error in replication `~s` (triggered by document `~s`): ~s"
...
stop_all_replications() ->
    ?LOG_REP("Stopping all ongoing replications because the replicator"
...
update_rep_doc(RepDocId, KVs) ->
    ...
    try
        ...
        ?LOG_REP_ERROR("Conflict error when updating replication document `~s`."
            " Retrying.", [RepDocId]),
        ok = timer:sleep(5),
        update_rep_doc(RepDocId, KVs)
    after
