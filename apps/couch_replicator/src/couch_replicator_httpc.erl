...
process_response({error, sel_conn_closed}=Error, _Worker, HttpDb, Params, _Cb) ->
    ?LOG_REP_DEBUG("server closed the socket ~p~n", [Error]),
    throw({retry, HttpDb, Params});

process_response({error, {'EXIT', {normal, _}}}=Error, _Worker, HttpDb ,Params, _Cb) ->
...
do_redirect(_Worker, Code, Headers, #httpdb{url = Url} = HttpDb, Params,
            _Cb) ->
